...
await build({
  stdin: {
    contents: `
      ...
      export { pageBySlug, PAGES } from "@/editor-source/site/pageMap";
    `,
    resolveDir: ROOT,
    loader: "ts",
  },
...
const { buildDoc, pageToc, pageBySlug, PAGES } = await import(pathToFileURL(out).href);

const slugs = process.argv.slice(2).length ? process.argv.slice(2) : PAGES.map((p) => p.slug);
...
for (const slug of slugs) {
  ...
  const chosen = page.sections.map((a) => byAnchor.get(a)).filter(Boolean);
  console.log(`\n### ${slug} — ${page.title} (${page.module})`);
  for (const t of pageToc(chosen))
