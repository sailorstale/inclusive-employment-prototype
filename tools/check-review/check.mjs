...
// ── 1б. Картинки, снятые со страницы ────────────────────────────────────────
/*
  У картинки нет текста, поэтому правилом по тексту её не снять — она
  адресуется адресом файла (CUT_IMAGES в dropScaffold.ts). Ошибиться тут легче,
  чем кажется: адрес пишется руками, а промах ничего не ломает и ничем себя не
  выдаёт. Картинка просто остаётся на странице, хотя клиент просила её убрать.

  Обратный случай тоже возможен: один файл вставлен на сайте дважды, и снятие
  уберёт обе картинки, включая ту, которую никто не просил трогать.

  Поэтому каждый адрес обязан найти ровно одну картинку в источнике.
*/
const { CUT_IMAGES } = await importTs("src/editor-source/site/dropScaffold.ts");
...
async function sourceImages() {
  ...
  for (const file of (await fs.readdir(dir)).filter((f) => f.endsWith(".ts"))) {
    const raw = await fs.readFile(path.join(dir, file), "utf8");
    for (const m of raw.matchAll(/"src":\s*"((?:[^"\\]|\\.)*)"/g)) {
...
for (const src of CUT_IMAGES) {
  const n = images.get(src) ?? 0;
  if (n === 1) continue;
  note(
    n === 0
      ? `Снятие картинки не находит её в источнике:\n    «${src}»\n    Адрес файла изменился или картинку уже убрали — правило больше ничего не делает.`
      : `Снятие картинки убирает ${n} картинок сразу:\n    «${src}»\n    Тот же файл вставлен на сайте несколько раз, и пропадут все.`,
  );
}
