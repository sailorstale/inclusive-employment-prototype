...
/*
  ...
  вместе с остальной разметкой. Так случилось 11 августа 2026 с заданием на
  «Запуске программы»: правка выглядела рабочей и не работала.

  Проверяем ровно этот случай: правка ставит на заголовок разметку, а канон
  этот заголовок переписывает. Ссылку в таком месте надо ставить не в заголовок,
  а отдельным блоком рядом, как это сделано на «Дорожной карте».
*/
...
for (const page of PAGES) {
  ...
  for (const [key, next] of headingEdits) {
    if (!next?.md || !hasMarkup(next.md)) continue;
    if (canonize(next.text) === next.text) continue;
    note(
