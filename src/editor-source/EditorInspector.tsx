...
export function EditorInspector({ docked = false }: { docked?: boolean } = {}) {
  const {
    editorMode,
    storeMode,
    active,
  ...
  const variants = active
    ? active.page?.startsWith("/source")
      ? getSourceVariantsFor(active.original)
      : getVariantsFor(active.original)
    : undefined;
  const record = active ? edits[active.id] : undefined;

  const { commentOf, setComment, setDeleted } = useComments();
  const annotation = active ? commentOf(active.id) : undefined;

  const [tab, setTab] = React.useState<Tab>("custom");
  const [draft, setDraft] = React.useState("");
  const [comment, setCommentDraft] = React.useState("");
  ...
  React.useEffect(() => {
    ...
    setCommentDraft(commentOf(active.id)?.text ?? "");
    setShowDiff(true);
  ...
  // Встроенная колонка (docked) всегда на месте: при отсутствии выбора/режима —
  // плейсхолдер. Плавающая панель (сайт) при этом просто скрыта (null).
  if (!editorMode || !active) {
    if (!docked) return null;
    return (
      <aside
        aria-label="Редактор блока"
        className="flex h-full min-h-0 flex-col border-l bg-card text-muted-foreground"
      >
        <div className="flex flex-1 items-center justify-center p-6 text-center text-sm">
          {!editorMode
            ? "Включите режим редактора (карандаш внизу слева), затем выберите блок."
            : "Выберите блок в источнике — клик по тексту, — чтобы отредактировать."}
        </div>
      </aside>
    );
  }
  ...
  const commentChanged = comment.trim() !== (annotation?.text ?? "").trim();

  const Wrapper = docked ? DockedWrapper : OverlayWrapper;
  return (
    <Wrapper onClose={closeInspector}>
        <div className="flex items-center justify-between border-b px-4 py-3">
          <div className="flex flex-wrap items-center gap-2 text-sm font-medium">
            ...
            {annotation?.text ? (
              <span className="inline-flex items-center gap-1 rounded-full bg-[hsl(var(--warn)/0.15)] px-2 py-0.5 text-[11px] font-medium text-[hsl(var(--warn))]">
                <MessageSquare className="h-3 w-3" /> с комментарием
              </span>
            ) : null}
            {annotation?.deleted ? (
              <span className="inline-flex items-center gap-1 rounded-full bg-[hsl(var(--bad)/0.12)] px-2 py-0.5 text-[11px] font-medium text-[hsl(var(--bad))]">
                <Trash2 className="h-3 w-3" /> помечен удалён
              </span>
            ) : null}
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4">
          ...
          {/* Комментарий к блоку и пометка удаления — часть содержимого панели
              (прокручиваются вместе с текстом, не «липкий» футер внизу). */}
          <div className="mt-6 space-y-3 border-t pt-4">
            <div>
              ...
              <textarea
                ...
                rows={2}
                placeholder="Заметка редактору/разработчику — что не так с этим блоком…"
                className="w-full resize-y rounded-md border bg-background px-3 py-2 text-sm leading-relaxed focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
              />
              {commentChanged ? (
                <button
                  type="button"
                  onClick={() => setComment(active, comment)}
                  className="mt-2 inline-flex items-center gap-1.5 rounded-md bg-brand px-3 py-1.5 text-sm font-medium text-brand-foreground hover:bg-[hsl(var(--brand)/0.9)]"
                >
                ...
                <p className="mt-1.5 text-[11px] text-muted-foreground">
                  Комментарий сохранён · виден в «Изменениях».
                </p>
              ) : null}
            </div>
            ...
            <button
              type="button"
              onClick={() => setDeleted(active, !annotation?.deleted)}
              className={cn(
                "inline-flex w-full items-center justify-center gap-1.5 rounded-md border px-3 py-2 text-sm font-medium transition-colors",
                annotation?.deleted
                  ? "border-[hsl(var(--ok)/0.4)] text-[hsl(var(--ok))] hover:bg-[hsl(var(--ok)/0.08)]"
                  : "border-[hsl(var(--bad)/0.35)] text-[hsl(var(--bad))] hover:bg-[hsl(var(--bad)/0.08)]"
              )}
            >
              {annotation?.deleted ? (
                <>
                  <Undo2 className="h-4 w-4" />
                  Вернуть блок из удалённых
                </>
              ) : (
                <>
...
// Плавающая панель (сайт): затемнение + фикс. панель справа.
...
// Встроенная колонка (инструмент источника): всегда на месте, без затемнения.
function DockedWrapper({ children }: { onClose: () => void; children: React.ReactNode }) {
  return (
    <aside
      aria-label="Редактор блока"
      className="flex h-full min-h-0 flex-col border-l bg-card text-card-foreground"
    >
      {children}
    </aside>
  );
