...
import type { EditRecord, EditStatus, Variant } from "@/editor/types";
...
export function EditorProvider({
  ...
  /** Скоуп хранилища: сайт или обособленный инструмент «Редактура источника». */
  scope?: Scope;
}) {
  ...
  // Первичная загрузка правок (для своего скоупа).
  React.useEffect(() => {
    let alive = true;
    loadEdits(scope).then((map) => {
      if (!alive) return;
      setEdits(map);
      setStoreMode(getMode(scope));
      setLoaded(true);
    });
    return () => {
      alive = false;
  ...
  const revert = React.useCallback(
    (id: string) => {
      ...
      if (rec && (rec.status === "applied" || rec.status === "verified")) {
        setStatusRemote(id, "rollback", scope)
          .then((updated) => {
  ...
  const setStatus = React.useCallback(
    (id: string, status: EditStatus) => {
      setStatusRemote(id, status, scope)
        .then((rec) => {
