...
export const bibleBlocks: BibleBlock[] = [
  ...
  {
    "id": "bible::koren::para::19a33el",
    "kind": "para",
    "md": "Вся выгрузка — один файл, один объект с одним полем. Так устроен файл разработчика, и держаться нужно ровно его структуры: 9 сентября 2026 Евгения загрузила нашу выгрузку, собранную иначе, и с сайта пропали меню и фотографии.",
    "text": "Вся выгрузка — один файл, один объект с одним полем. Так устроен файл разработчика, и держаться нужно ровно его структуры: 9 сентября 2026 Евгения загрузила нашу выгрузку, собранную иначе, и с сайта пропали меню и фотографии."
  },
  {
    "id": "bible::koren::code::1huod2j",
    "kind": "code",
    "lang": "json",
    "body": "{\n  \"pages\": []\n}",
    "text": "{\n  \"pages\": []\n}"
  },
  {
    ...
    "rows": [
      [
        "`pages`",
        "Массив страниц вместе с записями разделов меню. Сейчас 33 записи: 29 страниц и 4 раздела."
      ]
    ],
    "text": "Поле | Что это | pages | Массив страниц вместе с записями разделов меню. Сейчас 33 записи: 29 страниц и 4 раздела."
  },
  {
    "id": "bible::koren::para::yfecsf",
    "kind": "para",
    "md": "Поле `section` («Сайт») из корня убрано 9 сентября 2026: у разработчика его нет.",
    "text": "Поле section («Сайт») из корня убрано 9 сентября 2026: у разработчика его нет."
  },
  {
    "id": "bible::koren::para::xw81tw",
    "kind": "para",
    "md": "**Раздел меню — запись без содержимого.** Перед страницами каждого раздела в `pages` стоит запись из двух полей: адрес и подпись. По ним конструктор строит верхнее меню сайта, и без них меню пропадает. Своей страницы у раздела нет: «Основы» и «Для компаний» открываются первой страницей раздела, «Для соискателей» пока пуст и стоит в конце списка ([siteExport.ts](prototype/src/editor-source/site/siteExport.ts), `withHubs`).",
    "text": "Раздел меню — запись без содержимого. Перед страницами каждого раздела в pages стоит запись из двух полей: адрес и подпись. По ним конструктор строит верхнее меню сайта, и без них меню пропадает. Своей страницы у раздела нет: «Основы» и «Для компаний» открываются первой страницей раздела, «Для соискателей» пока пуст и стоит в конце списка (siteExport.ts, withHubs)."
  },
  {
    "id": "bible::koren::code::ym66r9",
    "kind": "code",
    "lang": "json",
    "body": "{ \"slug\": \"/general\", \"navTitle\": \"Основы\" }",
    "text": "{ \"slug\": \"/general\", \"navTitle\": \"Основы\" }"
  },
  {
    "id": "bible::koren::para::1qv7moy",
    "kind": "para",
    "md": "Порядок в файле: `/general` и девять страниц «Основ», `/companies` и шесть шагов, `/ngo` и четырнадцать страниц НКО, `/jobseekers`.",
    "text": "Порядок в файле: /general и девять страниц «Основ», /companies и шесть шагов, /ngo и четырнадцать страниц НКО, /jobseekers."
  },
  {
  ...
  {
    "id": "bible::5-tekst-edet-s-tegami-a-ne-v-markdown::table::xdegdm",
    "kind": "table",
    "header": [
      "Что было",
      "Что приезжает"
    ...
    "rows": [
      ...
      [
        "ссылка наружу",
        "`<a href=\"https://trudvsem.ru/\" target=\"_blank\" rel=\"noreferrer external\">Службы занятости</a>`"
      ],
      [
        "ссылка внутрь сайта",
        "`<a href=\"/general/quotas\" target=\"_blank\" rel=\"noopener\">квоты и господдержка</a>`"
      ],
      [
    ...
    "text": "Что было | Что приезжает | жирный | <b>жирный</b> | *курсив* | <i>курсив</i> | ссылка наружу | <a href=\"https://trudvsem.ru/\" target=\"_blank\" rel=\"noreferrer external\">Службы занятости</a> | ссылка внутрь сайта | <a href=\"/general/quotas\" target=\"_blank\" rel=\"noopener\">квоты и господдержка</a> | якорь внутри страницы | <a href=\"#glossary\">словарь</a> | подсказка к термину | <tooltip title=\"ГИТ\" description=\"Государственная инспекция труда…\">ГИТ</tooltip> | перенос строки внутри пункта списка | <br> | несколько абзацев в одном поле | <p>…</p><p>…</p> | перечисление внутри поля | <ul><li>…</li></ul>"
  },
  {
  ...
  {
    "id": "bible::5-tekst-edet-s-tegami-a-ne-v-markdown::para::x73rhe",
    "kind": "para",
    "md": "**У четырёх полей правило другое** — там абзац обёрнут всегда, даже когда он один. Это `text` у цитаты (`Quote`), `text` у врезки (`Phrase`), `description` и `explanation` у квиза. Разнобой возникал именно у них: рядом стоят и длинные тексты в несколько абзацев со списком внутри, и короткие в одну фразу, поэтому часть приезжала с тегами, а часть без.",
    "text": "У четырёх полей правило другое — там абзац обёрнут всегда, даже когда он один. Это text у цитаты (Quote), text у врезки (Phrase), description и explanation у квиза. Разнобой возникал именно у них: рядом стоят и длинные тексты в несколько абзацев со списком внутри, и короткие в одну фразу, поэтому часть приезжала с тегами, а часть без."
  },
  {
    "id": "bible::5-tekst-edet-s-tegami-a-ne-v-markdown::para::zy0iza",
    "kind": "para",
    "md": "Признак, по которому поле попадает в этот список, простой: **если у поля бывает несколько абзацев, тег ставится всегда**. Где абзац один по природе — у заголовка, пункта списка, названия карточки, вопроса квиза, — поле остаётся голой строкой, отделять там нечего.",
    "text": "Признак, по которому поле попадает в этот список, простой: если у поля бывает несколько абзацев, тег ставится всегда. Где абзац один по природе — у заголовка, пункта списка, названия карточки, вопроса квиза, — поле остаётся голой строкой, отделять там нечего."
  },
  {
    "id": "bible::5-tekst-edet-s-tegami-a-ne-v-markdown::para::1t7sa3d",
    "kind": "para",
    "md": "Вопрос квиза стоит отметить отдельно. Один абзац у него не по счастливой случайности, а по сборке: если у вопроса набирается несколько абзацев, они склеиваются в один. Так вышло с ситуационным квизом на «Шаге 5», где ситуация описана внутри вопроса и не уместилась в одну фразу. Поэтому на голую строку в этом поле можно опираться.",
    "text": "Вопрос квиза стоит отметить отдельно. Один абзац у него не по счастливой случайности, а по сборке: если у вопроса набирается несколько абзацев, они склеиваются в один. Так вышло с ситуационным квизом на «Шаге 5», где ситуация описана внутри вопроса и не уместилась в одну фразу. Поэтому на голую строку в этом поле можно опираться."
  },
  {
    "id": "bible::5-tekst-edet-s-tegami-a-ne-v-markdown::para::c3hb4j",
    "kind": "para",
    "md": "У цитаты правило касается только поля `text`: имя, должность и организация остаются обычными строками.",
    "text": "У цитаты правило касается только поля text: имя, должность и организация остаются обычными строками."
  },
  {
  ...
  {
    "id": "bible::6-ssylka-edet-gotovoy-s-protokolom-i-s-atributami::heading::1x6l2il",
    "kind": "heading",
    "level": 3,
    "md": "6. Ссылка едет готовой: с протоколом и с атрибутами",
    "text": "6. Ссылка едет готовой: с протоколом и с атрибутами",
    "anchor": "6-ssylka-edet-gotovoy-s-protokolom-i-s-atributami"
  },
  {
    "id": "bible::6-ssylka-edet-gotovoy-s-protokolom-i-s-atributami::para::16fcn0k",
    "kind": "para",
    "md": "Правило простое и мы держим его с нашей стороны: **внешняя ссылка всегда с протоколом** (`https://…`), **внутренняя всегда путь от корня** (`/general/quotas`).",
    "text": "Правило простое и мы держим его с нашей стороны: внешняя ссылка всегда с протоколом (https://…), внутренняя всегда путь от корня (/general/quotas)."
  },
  {
    "id": "bible::6-ssylka-edet-gotovoy-s-protokolom-i-s-atributami::para::drplng",
    "kind": "para",
    "md": "С 30 августа 2026 тег ссылки уходит на страницу **как есть**, без обработки на стороне разработчика, поэтому атрибуты `target` и `rel` стоят прямо в данных (журнал изменений разработчика, раздел 2):",
    "text": "С 30 августа 2026 тег ссылки уходит на страницу как есть, без обработки на стороне разработчика, поэтому атрибуты target и rel стоят прямо в данных (журнал изменений разработчика, раздел 2):"
  },
  {
    "id": "bible::6-ssylka-edet-gotovoy-s-protokolom-i-s-atributami::table::1wui5u0",
    "kind": "table",
    "header": [
      "Тип ссылки",
      "Как едет",
    ...
    "rows": [
      [
        ...
        "новая вкладка, стрелка у ссылки"
      ],
      [
        "внутрь сайта (`/…`)",
        "`target=\"_blank\" rel=\"noopener\"`",
        "новая вкладка, обычный вид"
      ],
      [
        "якорь внутри страницы (`#…`)",
        "без атрибутов",
    ...
    "text": "Тип ссылки | Как едет | Что получает читатель | наружу (https://…) | target=\"_blank\" rel=\"noreferrer external\" | новая вкладка, стрелка у ссылки | внутрь сайта (/…) | target=\"_blank\" rel=\"noopener\" | новая вкладка, обычный вид | якорь внутри страницы (#…) | без атрибутов | переход по странице"
  },
  {
    "id": "bible::6-ssylka-edet-gotovoy-s-protokolom-i-s-atributami::para::7nfyl6",
    "kind": "para",
    "md": "Стрелку у внешней ссылки разработчик рисует по слову `external` в `rel` — никакого класса или вложенного тега писать не надо. Внутренние ссылки открываются в новой вкладке **сознательно** (ответ Евгении от 9 сентября 2026), и `noreferrer` им не ставится: реферер нужен Метрике. Почтовые ссылки (`mailto:`) договором не описаны — на сайте их нет, а если появятся, поедут без атрибутов. Правило собирает [contentTree.ts](prototype/src/editor-source/source/contentTree.ts) (функция `linkAttrs`), а сторож на странице `/checks` сверяет с ним каждую ссылку в выгрузке.",
    "text": "Стрелку у внешней ссылки разработчик рисует по слову external в rel — никакого класса или вложенного тега писать не надо. Внутренние ссылки открываются в новой вкладке сознательно (ответ Евгении от 9 сентября 2026), и noreferrer им не ставится: реферер нужен Метрике. Почтовые ссылки (mailto:) договором не описаны — на сайте их нет, а если появятся, поедут без атрибутов. Правило собирает contentTree.ts (функция linkAttrs), а сторож на странице /checks сверяет с ним каждую ссылку в выгрузке."
  },
  {
    "id": "bible::6-ssylka-edet-gotovoy-s-protokolom-i-s-atributami::para::hslinn",
    "kind": "para",
  ...
  {
    "id": "bible::6-ssylka-edet-gotovoy-s-protokolom-i-s-atributami::para::1rnbwws",
    "kind": "para",
  ...
  },
  {
    ...
    "rows": [
      ...
      [
        ...
        "`L` · `M`",
        "да",
        "Размер пунктов. Задаётся местом, как у `Text`: в тексте страницы `L`, внутри `General Card`, `Accordion` и ячейки таблицы — `M`."
      ],
      [
        "`icon`",
        "ключ Lucide",
    ...
    "text": "Поле | Значения | Обяз. | Что значит | component | \"Stack\" | да |  | marker | dot · icon · number | да | Маркер пунктов. Точка — 418 списков, иконка — 10, номер — 7. | size | L · M | да | Размер пунктов. Задаётся местом, как у Text: в тексте страницы L, внутри General Card, Accordion и ячейки таблицы — M. | icon | ключ Lucide | нет | Только при marker: \"icon\". Если поля нет — галочка. | children | массив узлов | да | Обычно List Item."
  },
  {
  ...
  {
    "id": "bible::general-card::table::it6uip",
    "kind": "table",
    "header": [
      "Поле",
      "Значения",
  ...
  {
    "id": "bible::general-card::code::7gauqg",
    "kind": "code",
    "lang": "json",
  ...
  {
    "id": "bible::accordion::para::f2j7zf",
    "kind": "para",
    "md": "Весь текст ответа размера `M`: и абзацы, и списки, и списки внутри белой карточки (правило у `Stack`). Проверка `размер-внутри-компонента` ловит, если внутрь просочился `L`.",
    "text": "Весь текст ответа размера M: и абзацы, и списки, и списки внутри белой карточки (правило у Stack). Проверка размер-внутри-компонента ловит, если внутрь просочился L."
  },
  {
  ...
  },
  {
    ...
    "rows": [
      ...
      [
        ...
        "адрес картинки на хостинге разработчика",
        "да",
        "Например `https://avatars.mds.yandex.net/get-lpc/…/orig?width=1536&height=1604`."
      ],
      [
        "`alt`",
        "строка",
        "нет",
        "Подпись для скринридера. Сейчас её нет ни у одной из шести картинок — это дырка в содержании, закрыть её можно только руками."
      ]
    ],
    "text": "Поле | Значения | Обяз. | Что значит | component | \"Image\" | да |  | src | адрес картинки на хостинге разработчика | да | Например https://avatars.mds.yandex.net/get-lpc/…/orig?width=1536&height=1604. | alt | строка | нет | Подпись для скринридера. Сейчас её нет ни у одной из шести картинок — это дырка в содержании, закрыть её можно только руками."
  },
  {
    "id": "bible::image::para::bgra3d",
    "kind": "para",
    "md": "С 9 сентября 2026 в `src` едет готовый адрес хостинга, а не путь прототипа: разработчик залил все шесть схем к себе и прислал адреса. Внутри прототипа файл по-прежнему лежит в `public/source-media/<модуль>/`, и страница показывает его оттуда, а подмена на адрес хостинга делается только при выгрузке по таблице в [hostedImages.ts](prototype/src/editor-source/site/hostedImages.ts). Новую схему разработчик сначала заливает к себе, потом её адрес заводится в таблицу; пока адреса нет, картинка едет путём прототипа, и сторож на `/checks` это показывает (правило `картинка-не-на-хостинге`).",
    "text": "С 9 сентября 2026 в src едет готовый адрес хостинга, а не путь прототипа: разработчик залил все шесть схем к себе и прислал адреса. Внутри прототипа файл по-прежнему лежит в public/source-media/<модуль>/, и страница показывает его оттуда, а подмена на адрес хостинга делается только при выгрузке по таблице в hostedImages.ts. Новую схему разработчик сначала заливает к себе, потом её адрес заводится в таблицу; пока адреса нет, картинка едет путём прототипа, и сторож на /checks это показывает (правило картинка-не-на-хостинге)."
  },
  {
    "id": "bible::image::para::4g8gah",
    "kind": "para",
    "md": "**Портреты и логотипы едут так же — адресом хостинга.** До 9 сентября 2026 в этих полях стояло голое имя файла («master-ok»), и разработчик подставлял адреса руками; при загрузке нашей выгрузки все фото с сайта пропали. Теперь имя подменяется адресом при выгрузке по таблице в [hostedPhotos.ts](prototype/src/editor-source/site/hostedPhotos.ts), собранной из журнала изменений разработчика. Два исключения: `yandex` остаётся именем по решению разработчика, а портрет, которого в таблице нет, едет именем, и сторож это показывает (правило `портрет-не-на-хостинге`) — значит, файл разработчику ещё не отдан.",
    "text": "Портреты и логотипы едут так же — адресом хостинга. До 9 сентября 2026 в этих полях стояло голое имя файла («master-ok»), и разработчик подставлял адреса руками; при загрузке нашей выгрузки все фото с сайта пропали. Теперь имя подменяется адресом при выгрузке по таблице в hostedPhotos.ts, собранной из журнала изменений разработчика. Два исключения: yandex остаётся именем по решению разработчика, а портрет, которого в таблице нет, едет именем, и сторож это показывает (правило портрет-не-на-хостинге) — значит, файл разработчику ещё не отдан."
  },
  {
  ...
  },
  {
    ...
    "rows": [
      ...
      [
        "`href`",
        "адрес встраиваемого плеера Яндекса с размерами ролика в хвосте",
        "да фактически"
      ],
      [
        "`aspectRatio`",
        "пропорции ролика **со слэшем**: `\"2/3.5\"`, `\"9/16\"`, `\"1/1\"`",
        "нет — без поля 16/9"
      ],
      [
        "`poster`",
        "адрес кадра-обложки для плеера",
        "нет — пока не заполнен ни у одного ролика"
      ]
    ],
    "text": "Поле | Значения | Обяз. | component | \"Video\" | да | href | адрес встраиваемого плеера Яндекса с размерами ролика в хвосте | да фактически | aspectRatio | пропорции ролика со слэшем: \"2/3.5\", \"9/16\", \"1/1\" | нет — без поля 16/9 | poster | адрес кадра-обложки для плеера | нет — пока не заполнен ни у одного ролика"
  },
  {
    "id": "bible::video::code::1q91rte",
    "kind": "code",
    "lang": "json",
    "body": "{ \"component\": \"Video\", \"href\": \"https://frontend.vh.yandex.ru/player/vplvyifupiipo2ofssit?width=720&height=1280\", \"aspectRatio\": \"2/3.5\" }",
    "text": "{ \"component\": \"Video\", \"href\": \"https://frontend.vh.yandex.ru/player/vplvyifupiipo2ofssit?width=720&height=1280\", \"aspectRatio\": \"2/3.5\" }"
  },
  {
    "id": "bible::video::para::2gai18",
    "kind": "para",
    "md": "С 30 августа 2026 ролики едут не файлом на Яндекс Диске, а адресом плеера `frontend.vh.yandex.ru/player/…` — так у разработчика (журнал изменений, раздел 5). Пары «файл на Диске → плеер» лежат в [videoPlayers.ts](prototype/src/editor-source/site/videoPlayers.ts); новый ролик заводится там же, иначе он уедет старым адресом Диска, и сторож это покажет. Пропорции нужны только портретным роликам: сейчас это два на «Шаге 1». Формат со слэшем, как в CSS, — окончательный (ответ Евгении от 9 сентября 2026); вариант через букву `x` из первой версии журнала отменён. Пустое значение разработчик заменяет умолчанием 16/9.",
    "text": "С 30 августа 2026 ролики едут не файлом на Яндекс Диске, а адресом плеера frontend.vh.yandex.ru/player/… — так у разработчика (журнал изменений, раздел 5). Пары «файл на Диске → плеер» лежат в videoPlayers.ts; новый ролик заводится там же, иначе он уедет старым адресом Диска, и сторож это покажет. Пропорции нужны только портретным роликам: сейчас это два на «Шаге 1». Формат со слэшем, как в CSS, — окончательный (ответ Евгении от 9 сентября 2026); вариант через букву x из первой версии журнала отменён. Пустое значение разработчик заменяет умолчанием 16/9."
  },
  {
  ...
  {
    "id": "bible::person-item::table::1l3824e",
    "kind": "table",
    "header": [
      "Поле",
      "Значения",
    ...
    "rows": [
      ...
      [
        ...
        "адрес картинки на хостинге разработчика",
        "да",
        "Как у цитат: имя файла подменяется адресом при выгрузке (`hostedPhotos.ts`). Снимки квадратные, 800 на 800 точек, рассчитаны на круг."
      ],
      [
        "`role`",
        "строка",
    ...
    "text": "Поле | Значения | Обяз. | Что значит | component | \"Person Item\" | да |  | name | строка | да | Имя и фамилия. | photo | адрес картинки на хостинге разработчика | да | Как у цитат: имя файла подменяется адресом при выгрузке (hostedPhotos.ts). Снимки квадратные, 800 на 800 точек, рассчитаны на круг. | role | строка | да | Должность."
  },
  {
  ...
  },
  {
    ...
    "rows": [
      ...
      [
        ...
        "строка с тегами",
        "нет",
        "Вопрос. Всегда один абзац, поэтому едет голой строкой, без `<p>`. Есть у 50 квизов из 55: у квиза-темы вопрос целиком в заголовке."
      ],
      [
        "`title`",
        "строка с тегами",
      ...
      [
        ...
        "строка с тегами",
        "нет",
        "Разбор после проверки. Абзац всегда обёрнут в `<p>`, даже если он один. Есть у 43."
      ]
    ],
    "text": "Поле | Значения | Обяз. | Что значит | component | \"Quiz\" | да |  | items | массив вариантов | да | См. ниже. | mode | single · multiple | да | Один верный вариант или несколько. Руками не проставляется — считается по числу верных вариантов, поэтому разъехаться с содержанием не может. Из 55 квизов 46 с одним верным ответом, 9 с несколькими. | question | строка с тегами | нет | Вопрос. Всегда один абзац, поэтому едет голой строкой, без <p>. Есть у 50 квизов из 55: у квиза-темы вопрос целиком в заголовке. | title | строка с тегами | нет | Заголовок темы. Есть у 21. | description | строка с тегами | нет | Вводный текст перед вопросом, бывает из нескольких абзацев и списков. Абзац всегда обёрнут в <p>, даже если он один. Есть у 12. | explanation | строка с тегами | нет | Разбор после проверки. Абзац всегда обёрнут в <p>, даже если он один. Есть у 43."
  },
  {
    "id": "bible::quiz::para::cqbaiz",
    "kind": "para",
    "md": "**Текст внутри квиза набирается одним кеглем на всё поле, поля `size` у него нет.** Кегль задаёт само поле: `description` это Body L, `question` это Body L Bold, `explanation` и `feedback` это Body M. Теги внутри поля кегль не меняют: `<li>` в разборе это строка того же Body M, а не компонент `List Item` из текста страницы. До 8 сентября 2026 справочник об этом молчал, и на сайте пункты списка внутри разбора вышли крупнее абзацев рядом: абзацы собрали как пояснение, а пункты как обычный список сайта. Такая строка с абзацами и списком вместе есть в 11 квизах.",
    "text": "Текст внутри квиза набирается одним кеглем на всё поле, поля size у него нет. Кегль задаёт само поле: description это Body L, question это Body L Bold, explanation и feedback это Body M. Теги внутри поля кегль не меняют: <li> в разборе это строка того же Body M, а не компонент List Item из текста страницы. До 8 сентября 2026 справочник об этом молчал, и на сайте пункты списка внутри разбора вышли крупнее абзацев рядом: абзацы собрали как пояснение, а пункты как обычный список сайта. Такая строка с абзацами и списком вместе есть в 11 квизах."
  },
  {
  ...
  },
  {
    ...
    "rows": [
      ...
      [
        ...
        "строка с тегами",
        "нет",
        "Разбор конкретного варианта. Есть у 12 квизов, 40 вариантов. Начинается сразу с объяснения: слово «Верно.» или «Неверно.», с которого разбор начинался в источнике, из файла убрано — вердикт над разбором считается по `correct` (таблица выше), и слово шло бы дважды подряд."
      ]
    ],
    "text": "Поле варианта | Значения | Обяз. | Что значит | text | строка с тегами | да | Текст варианта. | correct | true | нет | Стоит только у верных. У неверных поля нет вовсе — false в файл не едет. | feedback | строка с тегами | нет | Разбор конкретного варианта. Есть у 12 квизов, 40 вариантов. Начинается сразу с объяснения: слово «Верно.» или «Неверно.», с которого разбор начинался в источнике, из файла убрано — вердикт над разбором считается по correct (таблица выше), и слово шло бы дважды подряд."
  },
  {
  ...
  },
  {
    ...
    "rows": [
      ...
      [
        "`ссылка-без-пометки`",
        "medium",
        "Атрибуты ссылки не по договору: у внешней нет `target=\"_blank\" rel=\"noreferrer external\"`, у внутренней нет `target=\"_blank\" rel=\"noopener\"`, у якоря атрибуты есть."
      ],
      [
    ...
    "text": "Правило | Степень | Что ловит | ссылка-пустая | high | Ссылка без адреса. | ссылка-заглушка | high | Адрес с localhost, example.com или TODO. | ссылка-в-никуда | high | Внутренняя ссылка ведёт на адрес, которого на сайте нет. | ссылка-протокол | high | Адрес не по правилу «внешняя с протоколом, внутренняя от корня». | ссылка-без-пометки | medium | Атрибуты ссылки не по договору: у внешней нет target=\"_blank\" rel=\"noreferrer external\", у внутренней нет target=\"_blank\" rel=\"noopener\", у якоря атрибуты есть. | ссылка-без-шифрования | low | Ссылка по http:// вместо https://."
  },
  {
  ...
  {
    "id": "bible::raskladka-i-otdelnye-komponenty::table::10nxjsw",
    "kind": "table",
    "header": [
      "Правило",
      "Степень",
    ...
    "rows": [
      ...
      [
        "`проза-в-конверте`",
        "medium",
        "В `Block` лежит только проза: заголовки, абзацы, фразы, списки. У разработчика конверт становится пустой карточкой вокруг списка."
      ],
      [
        "`якорь-дубль`",
        "high",
      ...
      [
        ...
        "medium",
        "Внутри `General Card` или `Accordion` абзац или список не `M`: у разработчика список выйдет крупнее абзаца рядом."
      ],
      [
        "`таблица-рваная`",
        "high",
      ...
      [
        "`видео-не-плеер`",
        "medium",
        "Ролик ведёт на файл Диска, а не на плеер Яндекса — не заведён в `videoPlayers.ts`."
      ],
      [
        "`картинка-не-на-хостинге`",
        "medium",
        "Схема едет путём прототипа `/source-media/…`, а не адресом хостинга — не заведена в `hostedImages.ts`."
      ],
      [
        "`портрет-не-на-хостинге`",
        "medium",
        "Портрет или логотип едет именем, а не адресом хостинга — файл разработчику не отдан (`hostedPhotos.ts`). Имя `yandex` не считается."
      ],
      [
        "`раздел-без-подписи`",
        "high",
        "У записи раздела меню нет `navTitle`."
      ],
      [
        "`видео-пропорции`",
        "medium",
        "`aspectRatio` не со слэшем (`\"2/3.5\"`)."
      ],
      [
    ...
    "text": "Правило | Степень | Что ловит | в-секции-без-конверта | medium | В раздел напрямую положили то, что нужно заворачивать в Block. | проза-в-конверте | medium | В Block лежит только проза: заголовки, абзацы, фразы, списки. У разработчика конверт становится пустой карточкой вокруг списка. | якорь-дубль | high | Такой якорь на странице уже занят. | якорь-пустой · якорь-не-латиница | medium | У заголовка нет якоря или он не из латинских букв, цифр и дефисов. | цитата-неполная | high | У цитаты нет организации, логотипа или текста. | организация-битая | high и medium | Непарная кавычка в названии, название со строчной буквы, имя автора внутри названия. | цитата-без-автора | medium | У цитаты нет ни имени, ни фото. | цвет-карточки | medium | bgColor не из семи цветов. | карточка-без-заголовка | medium | У карточки нет title — вероятно, заголовок остался первым абзацем внутри. | размер-внутри-компонента | medium | Внутри General Card или Accordion абзац или список не M: у разработчика список выйдет крупнее абзаца рядом. | таблица-рваная | high | В строке ячеек не столько, сколько колонок в шапке. | таблица-без-шапки · таблица-без-строк | high | Нет header или нет ни одной строки. | таблица-пустые-ячейки | medium | Пустые ячейки — обычно съехавшая разметка после потерянного объединения. | таблица-без-подписи · подпись-не-называет | medium | Нет caption либо подпись начинается со служебной строки и таблицу не называет. | квиз-без-вопроса · квиз-без-вариантов · квиз-без-верного | high | Читателю нечего отвечать, нет вариантов, ни один не отмечен верным. | квиз-режим | high и medium | mode разошёлся с числом верных вариантов. | квиз-пустой-вариант | high | Пустой вариант ответа. | квиз-дубль-варианта | medium | Вариант повторяется. | квиз-без-разбора | low | Нет explanation. | список-смешанный | high | Маркер контейнера не icon, а у части пунктов своя иконка — похоже, склеены два разных списка. | список-из-одного | low | Нумерованный список из одного пункта: счёт начнётся заново у следующего. | медиа-без-адреса | high | У картинки нет src, у видео нет href. | видео-не-плеер | medium | Ролик ведёт на файл Диска, а не на плеер Яндекса — не заведён в videoPlayers.ts. | картинка-не-на-хостинге | medium | Схема едет путём прототипа /source-media/…, а не адресом хостинга — не заведена в hostedImages.ts. | портрет-не-на-хостинге | medium | Портрет или логотип едет именем, а не адресом хостинга — файл разработчику не отдан (hostedPhotos.ts). Имя yandex не считается. | раздел-без-подписи | high | У записи раздела меню нет navTitle. | видео-пропорции | medium | aspectRatio не со слэшем (\"2/3.5\"). | картинка-без-подписи | low | У картинки нет alt. | человек-без-имени | high | У человека нет имени. | человек-без-фото | low | У человека нет фотографии."
  },
  {
