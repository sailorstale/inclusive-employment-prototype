...
export const bibleBlocks: BibleBlock[] = [
  ...
  {
    "id": "bible::5-tekst-edet-s-tegami-a-ne-v-markdown::para::1r64ipf",
    "kind": "para",
    "md": "Признак, по которому поле попадает в этот список, простой: **если у поля бывает несколько абзацев, тег ставится всегда**. Где абзац один по природе — у заголовка, пункта списка, названия карточки, — поле остаётся голой строкой, отделять там нечего.",
    "text": "Признак, по которому поле попадает в этот список, простой: если у поля бывает несколько абзацев, тег ставится всегда. Где абзац один по природе — у заголовка, пункта списка, названия карточки, — поле остаётся голой строкой, отделять там нечего."
  },
  {
