...
export const bibleBlocks: BibleBlock[] = [
  ...
  {
    "id": "bible::5-tekst-edet-s-tegami-a-ne-v-markdown::para::1hfzuva",
    "kind": "para",
    "md": "**У цитаты и врезки правило другое** — там абзац обёрнут всегда, даже когда он один. Разнобой возникал именно у них: рядом стоят и длинные тексты в несколько абзацев, и короткие в одну фразу, поэтому часть приезжала с тегами, а часть без. Смотри разделы `Quote` и `Phrase`. У цитаты правило касается только поля `text`: имя, должность и организация остаются обычными строками.",
    "text": "У цитаты и врезки правило другое — там абзац обёрнут всегда, даже когда он один. Разнобой возникал именно у них: рядом стоят и длинные тексты в несколько абзацев, и короткие в одну фразу, поэтому часть приезжала с тегами, а часть без. Смотри разделы Quote и Phrase. У цитаты правило касается только поля text: имя, должность и организация остаются обычными строками."
  },
  {
