...
/*
  ЧАСТНЫЕ КОМПАНИИ БЕЗ ЛОГОТИПА — решение дизайнера от 8 сентября 2026.

  Набор логотипов выгружен из реестра НКО: у фондов знак есть у каждого, и в
  строке авторства он на месте. Компании же в набор не входили — их знаки мы
  клали руками (Пятёрочка, Касперский, АШАН, Бургер Кинг), а где знака не было,
  по слагу лежал серый прямоугольник (Сбербанк, «Лемана ПРО», «Мон’дэлис»,
  Melon Fashion Group). Получалось вразнобой: у одних компаний настоящий
  логотип, у других заглушка. Дизайнер решил снять логотип у всех частных
  компаний: в выгрузке у таких цитат нет ключа `logo`, на странице нет ни
  знака, ни серого места под него. Имя, должность и название компании в
  должности остаются — по ним читатель и понимает, кто говорит.

  Знак Яндекса под правило не попадает: это не файл логотипа, а фирменный
  красный круг, и он часть строки авторства по решению заказчика.

  Записи в каталоге _index.json у компаний остаются: по ним раскладка находит
  название организации в должности («директор по персоналу АШАН Ритейл
  Россия») и пишет его в поле org. Убираем только логотип, не организацию.

  Список — по слагам каталога. Новая компания в цитате: завести запись в
  _index.json (чтобы нашлось название) и добавить слаг сюда.
*/
export const COMPANIES: ReadonlyArray<{ slug: string; name: string }> = [
  { slug: "ashan", name: "АШАН Ритейл Россия" },
  { slug: "burger-king", name: "Бургер Кинг Россия" },
  { slug: "laboratoriya-kasperskogo", name: "Лаборатория Касперского" },
  { slug: "lemana-pro", name: "Лемана ПРО" },
  { slug: "melon-fashion-group", name: "Melon Fashion Group" },
  { slug: "mondelis-v-rossii", name: "Мон’дэлис в России" },
  { slug: "pyaterochka", name: "Пятёрочка" },
  { slug: "sberbank", name: "Сбербанк" },
];

/** Слаг из каталога принадлежит частной компании — логотип не показываем. */
export const isCompanySlug = (slug?: string): boolean =>
  !!slug && COMPANIES.some((c) => c.slug === slug);

/*
  То же по названию организации — для сторожа выгрузки: у него на руках только
  готовый JSON, где слага уже нет. Сравниваем без учёта регистра, кавычек и «ё»
  (normalize), чтобы «Мон'дэлис» и «Мон’дэлис» считались одним названием.
*/
export const isCompanyOrg = (org: string): boolean => {
  const want = normalize(org);
  return !!want && COMPANIES.some((c) => normalize(c.name) === want);
};
