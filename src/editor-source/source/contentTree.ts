...
/** Компоненты, у которых поле text всегда размечено блоками. */
const ALWAYS_BLOCK_TEXT = new Set(["Quote", "Phrase"]);
      ...
      */
      ...
      if (TEXT_KEYS.has(k) && typeof v === "string") {
        /*
          ...
          У цитаты и врезки правило строже: абзац всегда в теге, даже одинокий
          (см. blockTextToTags). Проверяем И компонент, И имя поля: без второй
        ...
        */
        out[k] =
          component === "List Item"
            ? mdLinesToTags(v)
            : ALWAYS_BLOCK_TEXT.has(component ?? "") && k === "text"
              ? blockTextToTags(v)
              : mdBlocksToTags(v);
        continue;
      }
