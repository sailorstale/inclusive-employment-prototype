...
type NodeKind =
  ...
  | {
      ...
      */
      ...
      /*
        ...
        В ВЫГРУЗКУ ПОЛЕ НЕ ЕДЕТ (просьба разработчика 17 августа 2026). До этого
        ехало, чтобы он знал: место под фото не пустует, а отменено. Но у него
        правило и так простое — нет поля photo, значит нет ни фотографии, ни
        места под неё. Кружок-заглушка существует только в инструменте, это наше
  ...
  */
  ...
  /*
    ...
    себя тем, что открыл: в «Для НКО» это сотрудник НКО, в «Для компаний» —
    сотрудник компании. Заставлять его нажимать чип ещё раз незачем.
  */
  | { component: "Feedback"; roles?: string[]; defaultRole?: string }
...
const splitTitleBody = (text: string): { title?: string; body: string } => {
  ...
  if (bold) {
    const rest = text
      ...
      .replace(/^\s*:\s*/, "");
    /*
      Первую букву тела поднимаем — замечание клиента 7 августа 2026 к карточкам
      «Медицинский подход» и «Социальный подход». В источнике жирная фраза была
      подлежащим одного предложения: «**Медицинский подход** фокусируется на
      диагнозе». Заголовок карточки забрал подлежащее, и тело осталось началом
      с маленькой буквы — читается как опечатка.

      Симметрично соседней ветке (жирное за зачином): там по той же причине
      поднимается первая буква заголовка.
    */
    if (rest.trim())
      return {
...
/*
  ВЕРДИКТ АККОРДЕОНА — «Миф» или «Правда» отдельной строкой в начале ответа.

  Девять утверждений в разделе «Мифы об инклюзивном трудоустройстве» устроены
  одинаково: заголовок с утверждением, под ним строка-вердикт, дальше разбор. На
  странице вердикт оказывался ПОД захлопнутым вопросом, и списком читались одни
  утверждения без ответа — можно ли им верить, видно только раскрыв каждое.

  Замечание Мити от 12 августа 2026: вердикт должен стоять в начале самого
  утверждения, а отдельной строки быть не должно. Поэтому слово уезжает в вопрос
  аккордеона («Миф. Всем людям с инвалидностью нужны…»), а строка из тела
  уходит.

  Ловим обе формы, «Миф» и «Это миф»: первую пишет источник, вторую подставляет
  правка редакции по замечанию Юли от 7 августа 2026 (см. sourceEdits). Правку
  оставляем как есть — она про строку, которой на странице больше нет, а слово
  для вердикта мы берём сами.

  Сверка по всей строке целиком: слова «миф» и «правда» встречаются и в обычных
  предложениях, и такой абзац пропасть не должен.
*/
const VERDICT_LABEL = /^[*_\s]*(?:это\s+)?(миф|правда)[*_\s.:!]*$/iu;

const verdictWord = (t: string): string | undefined => {
  const m = VERDICT_LABEL.exec(t.trim());
  if (!m) return undefined;
  return m[1].toLowerCase() === "миф" ? "Миф" : "Правда";
};
...
export function splitQuizTop(title: string, parts: string[]): QuizTop {
  ...
  /*
    ВОПРОС — ВСЕГДА ОДИН АБЗАЦ (решение дизайнера 17 августа 2026). Обычно так и
    выходит само: вопросительный знак в конце последнего абзаца отделяет сам
    вопрос от вводной части, и она уезжает в description.

    Но у ситуационных квизов «Шага 5» вопросительного знака нет вовсе — там
    описана ситуация, а спрашивают уже варианты ответа. Тогда в вопрос идут все
    абзацы, и один из восьми не уместился в один: он приезжал разработчику
    двумя абзацами в тегах, пока остальные сорок девять шли голой строкой.

    Склеиваем пробелом, а не разрывом абзаца. Тогда поле question голое ВСЕГДА,
    и это гарантия формата, а не совпадение.
  */
...
*/
/*
  Подчёркивание внутри адреса ссылки — часть адреса, а не курсив. Адрес
  consultant.ru/document/cons_doc_LAW_8559/… терял оба подчёркивания и вёл в
  никуда (замечание Мити 16 сентября 2026). Поэтому адреса — хвост
  markdown-ссылки в скобках и голые https://… — правило курсива обходит.
*/
const stripItalic = (md: string) =>
  md
    .replace(/(^|[^*])\*([^*\n]+)\*(?!\*)/g, "$1$2")
    .split(/(\]\([^)]*\)|https?:\/\/\S+)/)
    .map((part, i) => (i % 2 ? part : part.replace(/(^|[^_])_([^_\n]+)_(?!_)/g, "$1$2")))
    .join("");
...
/*
  Слово ПЕРЕД кавычками, после которого в кавычках стоит не работодатель, а
  часть компании: направление, проект, программа, отдел. Падеж не разбираем —
  хватает начала слова.
*/
const UNIT_BEFORE_NAME =
  /(?:направлени|подразделени|отдел|департамент|управлени|служб|проект|программ|курс)\p{L}*\s*$/iu;

/*
  Правовая форма перед названием компании: «ПАО Сбербанк», «ООО Ромашка».
  Границу названия ставим по знаку препинания, по следующему слову со строчной
  буквы или по концу строки. Начало ищем от края строки, пробела или запятой:
  \b в кириллице не работает — он считает границей только латиницу.
*/
const LEGAL_FORM_NAME =
  /(?:^|[\s(,])(?:ПАО|ОАО|ЗАО|АО|ООО|НАО|ГК)\s+[«„"]?(\p{Lu}[^,«»„“"()]{1,40}?)[»“"]?(?=[,.;:]|\s+\p{Ll}|$)/u;

/*
  НАЗВАНИЕ РАБОТОДАТЕЛЯ В СТРОКЕ АВТОРСТВА, ГДЕ КАВЫЧЕК НЕСКОЛЬКО.

  Кавычки — признак имени собственного, но в кавычки берут не только компанию.
  «Виктория Кравцова, руководитель направления «Особенный банк для людей с
  инвалидностью» ПАО Сбербанк»: в кавычках направление внутри банка, а сам банк
  назван дальше и без кавычек. «Екатерина Фокина, руководитель направления
  «Разнообразие и инклюзия» компании «Лемана ПРО»»: кавычек две, компания во
  второй. Раньше бралась первая пара кавычек подряд — и подписью логотипа
  становилось название отдела.

  Различает их слово перед кавычками (см. UNIT_BEFORE_NAME): имя подразделения
  пропускаем и берём следующее. Не осталось ни одного — ищем название после
  правовой формы: она стоит только у компаний.
*/
const orgInAuthorLine = (line: string): string | undefined => {
  for (const m of line.matchAll(new RegExp(QUOTED_NAME.source, "gu"))) {
    if (UNIT_BEFORE_NAME.test(line.slice(0, m.index))) continue;
    return m[1].trim();
  }
  return LEGAL_FORM_NAME.exec(line)?.[1].trim();
};
  ...
  const groupNodes = (g: Group): Node[] => {
    ...
    */
    ...
    const forcedTitle = explicitTitle(dir) ?? labelToTitle(dir);
        ...
        for (const it of items) {
          ...
          */
          ...
          if (startsCard || !cards.length) {
            ...
            */
            /*
              Считается переносом и тогда, когда дизайнер написал не «Пример в
              заголовок», а «карточка с заголовком Пример»: слово в комментарии
              то же самое, что жирным стоит в начале абзаца. Раньше вторая
              формулировка считалась «заголовком со стороны», лид оставался в
              теле, и на карточке выходило «Пример / Пример. Слабослышащий
              сотрудник…» — слово подряд дважды.

              Заголовок из комментария, НЕ совпадающий с лидом, по-прежнему
              приходит со стороны: лид там настоящий текст, и терять его нельзя.
            */
            const movedLead =
              cards.length === 0 &&
              Boolean(lead) &&
              (sameLabel(labelToTitle(dir), lead![1]) ||
                sameLabel(explicitTitle(dir), lead![1]));
            /*
            ...
            */
            /*
              Жирный лид уехал в заголовок — первую букву оставшегося текста
              поднимаем. Замечание клиента 7 августа 2026 к карточкам
              «Медицинский подход» и «Социальный подход»: в источнике жирная
              фраза была подлежащим одного предложения («**Медицинский подход**
              фокусируется на диагнозе»), и после переноса тело начиналось со
              строчной буквы. Правило то же, что в splitTitleBody.
            */
      ...
      case "Accordion": {
        /*
          ТАБЛИЦА → АККОРДЕОНЫ. Строка таблицы даёт один аккордеон: первая
          ячейка становится вопросом, остальные — ответом. Имя колонки идёт
          перед своей ячейкой жирной строкой, иначе три куска текста читались бы
          одним потоком и было бы непонятно, где кончается канал связи и
          начинаются материалы.

          Зачем это нужно. Широкая таблица на странице не помещается: четыре
          колонки с абзацами текста уезжают вправо, и читатель либо крутит её
          вбок, либо не видит последнюю колонку вовсе. Аккордеон показывает
          сразу все группы кандидатов списком, а подробности открывает по
          нажатию — как раз то, что от таблицы и требовалось.

          Заголовков внутри ответа нет намеренно — то же решение дизайнера, что
          и ниже по ветке: настоящий заголовок здесь один, это сам вопрос.
        */
        const tb = items.length === 1 && items[0].b.kind === "table" ? items[0].b : null;
        if (tb && tb.header.length >= 2) {
          const wantRows = splitCount(dir);
          if (wantRows && tb.rows.length !== wantRows)
            return [
              {
                component: "note" as const,
                text: `просили ${wantRows} аккордеонов, а строк в таблице ${tb.rows.length} — проверьте деление`,
              },
            ];
          /*
            Ячейку разбираем тем же cellNodes, что и «таблица в текст»: он знает
            про пункты через «•» и про подзаголовки внутри ячейки. Размер текста
            опускаем до M — внутри компонента проза на ступень мельче.
          */
          const inner = (cell: string): Node[] =>
            cellNodes(applyFix(cell, fix)).map((n) =>
              n.component === "Text" ? { ...n, size: "M" as const } : n,
            );
          return tb.rows.map((row): Node => {
            const body: Node[] = [];
            row.slice(1).forEach((cell, i) => {
              if (!cell?.trim()) return;
              const col = tb.header[i + 1];
              if (col?.trim())
                body.push({
                  component: "Text",
                  size: "M",
                  text: `**${headingText(col, fix)}**`,
                });
              body.push(...inner(cell));
            });
            return {
              component: "Accordion",
              state: mods.state === "Expanded" ? "Expanded" : "Collapsed",
              question: headingText(applyFix(row[0] ?? "", fix), fix),
              children: body,
            };
          });
        }
        ...
        */
        ...
        for (const it of items2) {
          const isTop =
            ...
            !isBoldHeading(it);
          /*
            Вердикт («Миф», «Правда») стоит первой строкой ответа и уезжает в
            начало самого вопроса. Берём его только пока тело пусто: дальше по
            ответу то же слово может оказаться обычной фразой.
          */
          const verdict: string | undefined =
            q !== null && !body.length && !isTop
              ? verdictWord(md(it, fix))
              : undefined;
          if (isTop) {
            flush();
        ...
        starts.forEach((ai, k) => {
          ...
          */
          const quoted = hasAuthor
            ? orgInAuthorLine(stripEmph(parsed[ai].bare))
            : undefined;
          /*
      ...
      */
      case "Quiz": {
        ...
        */
        ...
        /*
          ...
          дверь?») без них меняют смысл.

          ЗАКАВЫЧЕННУЮ РЕПЛИКУ ПРАВИЛО НЕ ТРОГАЕТ, И ЭТО НАРОЧНО. Оно смотрит на
          самый последний знак строки, а у такой реплики это закрывающая
          кавычка, — точка стоит внутри неё и остаётся на месте. Заглядывать за
          кавычку правило не должно: в прямой речи точка обязательна, без неё
          реплика читается оборванной.

          Разбирать в кавычки варианты ответа целиком мы умеем в одном месте —
          «Разговор с работодателем» в треке «Для НКО» (проверено 10 августа
          2026 соседней сессией: двенадцать реплик, все с точками внутри
          кавычек). Так что случай не выдуманный, и «доработать» правило здесь
          значит сломать четыре квиза.
        ...
        if (items.some((it) => isSituationLabel(md(it, fix)))) {
          ...
          const out: Node[] = quizzes.map((q, i) => {
            ...
            /*
              ...
              Отдельного поля про кнопку «Проверить» здесь больше нет: с
              17 августа 2026 кнопка есть ровно у тех квизов, где верных
              вариантов несколько, и это видно по mode (см. Quiz.tsx).
            */
            const items = merged
      ...
      case "Image": {
        /*
          Картинка задаётся ПУТЯМИ ИЗ КОММЕНТАРИЯ директивы. Снимки приходят от
          клиента отдельно от текста (лежат на Яндекс Диске), а в источнике на их
          месте стоит одна пометка вроде «Фото экспертов» — забрать файл из самого
          абзаца тут неоткуда. Формат — строка на снимок:

            /figma/avatars/gulnara-gorishnyaya.jpg — Гульнара Горишняя, …

          Подпись после тире уходит в alt: без неё фотография для скринридера
          немая. Строк может быть несколько — тогда и картинок будет несколько:
          одна пометка в тексте нередко стоит за целой группой людей.

          Путей нет — отдаём рамку-заглушку, как и раньше: пометка в тексте есть,
          а файл ещё не прислали.
        */
        const shots = (dir.comment || "")
          .split(/\r?\n/)
          .map((line) =>
            /^(\/\S+\.(?:jpe?g|png|svg|webp))\s*(?:[—–-]\s*(.*))?$/iu.exec(line.trim()),
          )
          .filter((m): m is RegExpExecArray => Boolean(m))
          .map((m) => ({ src: m[1], alt: m[2]?.trim() }));
        return shots.length
          ? shots.map((s) => ({ component: "Image" as const, src: s.src, alt: s.alt }))
...
/*
  ВРЕЗКА — абзац ВСЕГДА в теге, даже когда он один. Два замечания разработчика
  от 17 августа 2026: сперва про цитаты, следом про фразу-врезку.

  Общее правило формата другое: одна фраза едет голой строкой, а теги
  появляются, только когда есть что разделять. Для абзаца, заголовка, пункта
  списка и ячейки таблицы это верно — там голые ВСЕ до одного, и разнобоя не
  возникает. Разнобой появлялся там, где рядом стоят и длинные тексты, и
  короткие: из 41 цитаты 10 приезжали голыми и 31 с тегами, из 7 врезок голой
  была одна. Вставить такой текст одним движением нельзя — сначала надо
  посмотреть, что прислали на этот раз.

  ТОЛЬКО ПОЛЕ РЕЧИ. Имя автора, должность и организация у цитаты — обычные
  строки, тегов в них быть не должно.
*/
export function blockTextToTags(text: string): string {
  return blocksToTags(textBlocks(text));
}

/*
  ПОЛЯ, РАЗМЕЧЕННЫЕ БЛОКАМИ ВСЕГДА: компонент → его поля.

  Список ведём по замечаниям разработчика, по одному полю за раз, а не «всё
  подряд»: у заголовка, пункта списка и названия карточки абзац один по природе,
  и тег там ничего не отделяет. Разнобой возникает только там, где поле бывает
  и в одну фразу, и в три абзаца со списком внутри.
*/
const ALWAYS_BLOCK_FIELDS: Record<string, string[]> = {
  Quote: ["text"],
  Phrase: ["text"],
  /*
    Вводный текст и разбор квиза — замечания разработчика 17 августа 2026.
    Вводный: 8 приезжали с тегами, 4 голыми. Разбор: 26 с тегами, 17 голыми.

    Правило он сформулировал сам: «если может быть несколько абзацев, то надо
    все оборачивать в теги». У обоих полей несколько абзацев бывают, значит
    тег ставится всегда. Вопрос (question) в список не входит по его же решению.
  */
  Quiz: ["description", "explanation"],
};

/** Всегда ли это поле размечается блоками. */
const alwaysBlock = (component: string | undefined, key: string): boolean =>
  Boolean(component && ALWAYS_BLOCK_FIELDS[component]?.includes(key));
...
const cleanForExport = (
  ...
  sink?: ExportPath[],
    ...
    for (const [rawKey, v] of Object.entries(n)) {
      ...
      /*
        noPhoto тоже остаётся внутри инструмента (просьба разработчика
        17 августа 2026). Оно говорит нам, что кружок-заглушку под будущий
        снимок рисовать не надо и напоминать «дополнить авторство» не о чем.
        Разработчику этого знать незачем: у него правило простое — нет поля
      ...
      */
      ...
      if (TEXT_KEYS.has(k) && typeof v === "string") {
        /*
          ...
          У части полей правило строже: абзац всегда в теге, даже одинокий (см.
          ALWAYS_BLOCK_FIELDS). Проверяем И компонент, И имя поля: без второй
          проверки в теги обернулись бы имя автора, должность и организация,
          и разработчику уехали «<p>Сергей Исаев</p>» и «<p>Яндекс</p>».
        */
        out[k] =
          component === "List Item"
            ? mdLinesToTags(v)
            : alwaysBlock(component, k)
              ? blockTextToTags(v)
              : mdBlocksToTags(v);
        continue;
      }
...
export function placeDirectives(
  ...
  /*
    ВЫКЛЮЧЕННАЯ ДИРЕКТИВА НЕ ОТБИРАЕТ БЛОК У ЖИВОЙ.

    На один блок иногда заведены две директивы. Так вышло с примером про
    слабослышащего сотрудника: сперва блок разметили карточкой, а следом завели
    вторую запись — просто чтобы оставить заметку «убери точку из заголовка».
    Цели у второй записи нет, и раскладка понимала её как «снять разметку».

    Раскладка раздавала блок по порядку, и последняя запись затирала первую.
    Абзац оставался и без карточки, и без обычного вида: на странице зияла
    пустота, а клиент написал «пример потерян» — и был прав.

    Поэтому при споре за один блок побеждает та, что вообще действует.
    Выключенная, отклонённая и ещё не принятая место занимает (иначе она
    пропала бы из панели разметки), но живую с него не сталкивает.
  */
  const place = (d: Directive, p: number) => {
    for (let k = 0; k < d.blocks.length; k++) {
      taken.add(p + k);
      const prev = map.get(flat[p + k].key);
      if (prev && isActive(prev) && !isActive(d)) continue;
