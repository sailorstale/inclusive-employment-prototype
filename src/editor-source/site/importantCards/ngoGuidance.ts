...
export const ngoGuidanceCards: PageCards = {
  ...
  cards: [
    ...
    */
    ...
    /*
      ЧЕТЫРЕ ИТОГА ЭТАПОВ НАЗЫВАЮТСЯ ПО СВОЕМУ ЭТАПУ — решение дизайнера от
      16 августа 2026: «вместо заголовков Важно надо написать Результат первого
      (второго, третьего, четвёртого) этапа». Оно же было и в его пометках к
      замечаниям клиента: «упаковываем в карточку жёлтую, используем заголовок
      который выделен жирным».

      Название переехало из текста в шапку карточки, поэтому из самого текста
      подпись убрана (см. rewrite в clientEdits/ngoGuidance.ts) — иначе одно и
      то же читалось бы дважды подряд. Вместе с текстом сменились и адреса
      блоков: прежние — …1ita6ey, …1721ag4, …bpn79r, …1qh2tx6.

      Жёлтый цвет задан полем bg. Обычно его даёт само название «Важно», а эти
      карточки называются иначе и без bg стали бы синими.

      Картинки «Важная информация» у них больше нет: её раскладка выдаёт тоже по
      названию «Важно». Для итога этапа она и не подходит — там не
      предупреждение, а вывод.
    */
    { blocks: [`${PLAN}::paragraph::wojox1`], title: "Результат первого этапа", bg: "yellow" },
