...
export function PageComments({
  ...
  */
  ...
  /*
    ...
    переключаем список на ту группу, где этот комментарий лежит. Иначе клик
    выглядел бы сломанным: рамка на странице подсветилась, а карточки в списке
    нет.

    Убранное в «Решено» требует отдельного разбора. Раньше здесь всегда
    ставилось «Все», но «Все» показывают только актуальное, и убранная карточка
                    ...
                    */}
                    ...
                    {!placed.has(g.id) && !done && !closed && (
                      /*
                        ...
                        У решённого замечания рамки нет намеренно — оно убрано
                        со страницы целиком, — поэтому пугать этой строкой там
                        не за что.
