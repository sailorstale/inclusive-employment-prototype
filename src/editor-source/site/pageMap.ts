...
export const OSNOVY_PAGES: OsnovyPage[] = [
  ...
  */
  ...
  // ── Для компаний: наём по шагам (М5) ────────────────────────────────────
  {
    ...
    */
    ...
    outline: {
      /*
        ЗАГОЛОВОК НАД НАЧАЛОМ МАТЕРИАЛА — замечание Юли msn9o4ot35n0 от
        11 августа 2026: над абзацем «Инклюзивный наём в компании начинается не
        с поиска кандидатов…» нужен заголовок «Шаг 1. Выбор вакансии для
        инклюзивного найма». Заголовок в источнике есть, но перекройка снимала
        его как повтор названия страницы (см. pageOutline): всё, что стоит до
        первого раздела, становится вступлением, а его заголовок пропадает.
        Названный разделом, он остаётся на месте и заодно открывает оглавление
        справа.

        Название в шапке при этом остаётся коротким («Шаг 1. Выбор вакансии») —
        решение дизайнера 11 августа 2026. В меню слева рядом стоят пять
        соседних шагов, и полное название вытеснило бы их.
      */
      sections: [
        "shag-1-vybor-vakansii-dlya-inklyuzivnogo-nayma",
      ...
      /*
        «Вакансия: Сборщик заказов на складе» — замечание Юли msn9zp4iae4l:
        «убрать из меню справа этот заголовок». На странице он остаётся: это
        подпись к разбору одной вакансии, по ней нельзя понять, о чём часть
        страницы, и она продолжает предыдущую мысль — ровно тот случай, под
        который заведены врезки (см. pageOutline).
      */
      inline: ["vakansiya-sborschik-zakazov-na-sklade"],
    },
  },
  {
  ...
  */
  ...
  },
  {
    ...
    /*
      «Почему важно заранее определить объём услуг» — замечание Юли от 11 августа
      2026: «убрать из меню справа». Заголовок остаётся на странице, но строкой в
      оглавлении он лишний: раздел «Работа с внешней аудиторией» там уже назван, а
      подзаголовок ведёт читателя туда же.
    */
    noToc: ["pochemu-vazhno-zaranee-opredelit-obem-uslug"],
  },
  {
    slug: "/ngo/candidates",
