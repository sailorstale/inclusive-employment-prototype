...
/*
  ...
  ВСЕ АБЗАЦЫ ВСТУПЛЕНИЯ ОБЫЧНОГО РАЗМЕРА — решение дизайнера 15 августа 2026.
  Раньше первый абзац приходил крупным (Text XL): в наборе компонентов это
  называлось лидом. Размера XL на сайте больше нет вовсе, и здесь ничего
  подменять не нужно — абзацы едут как есть.

  Лид при этом никуда не делся и остаётся опознаваемым: это первый текст первой
  секции, у которой нет ни заголовка, ни якоря. Изменился только его вид.
...
function introNodes(intro?: SectionNode): Node[] {
  ...
  return body[0]?.component === "Heading" ? body.slice(1) : body;
}
