  ...
  */
  const goToComment = React.useCallback(
    (id: string) => {
      ...
      /*
        Блок мог оказаться внутри свёрнутого аккордеона: тогда его нет в
        разметке вовсе, и ни рамку нарисовать, ни подвести страницу нельзя.
        Раскрываем всё свёрнутое — замечание должно показываться, а не
        объявляться потерянным из-за того, что читатель не догадался нажать.
      */
