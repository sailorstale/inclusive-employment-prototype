...
import { Boxes, Combine, ListChecks, PencilLine } from "lucide-react";
import { cn } from "@/lib/utils";
import { useEditor } from "./EditorProvider";
import { useComments } from "./CommentsProvider";

// Компактный плавающий док редакторских инструментов в левом нижнем углу:
// Редактор · Изменения. Вынесен из шапки, чтобы не мешать чтению и не влиять на
...
export function EditorDock() {
  ...
  // На «Изменениях» ждут внимания: новые/откатные правки + открытые комментарии
  // + помеченные удалёнными блоки.
  const pending =
  ...
  return (
    ...
    >
      ...
      <DockItem
        ...
        active={pathname === "/changes" || pathname === "/source/changes"}
        badge={pending}
        to={isSource ? "/source/changes" : "/changes"}
      />
      {!isSource && (
        <>
          <DockItem
