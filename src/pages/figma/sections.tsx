    ...
    <SectionContainer>
      ...
      <ListContainer>
        <ListItem size="L" type="Icon">
          List Item · L · Icon — маркер-галочка вместо точки
        </ListItem>
        <ListItem size="M" type="Icon">
          List Item · M · Icon — тот же маркер, кегль 16
        </ListItem>
  ...
  return (
    <SectionContainer>
      ...
      <Text size="L">
        ...
        Для обезличенной выделенной мысли берут обычную карточку. Длинная цитата
        уходит «под кат»: обрезается до пяти строк, ссылка «Далее» раскрывает
        остальное.
      </Text>
      ...
      <CardContainer>
        <Quote
          ...
          avatars={3}
        >
          Quote · L — акцентная цитата на кремовой подложке. Её задача — быть
          замеченной, поэтому она перебивает ритм колонки. А это уже длинный
          текст, чтобы показать кат: мы взяли первого сотрудника с инвалидностью,
          потому что подходили под квоту. Через полгода поняли, что он один из
          лучших в команде — и стали искать ещё. Сейчас у нас работает девять
          человек, и мы не собираемся останавливаться на достигнутом. Инклюзия
          оказалась не благотворительностью, а обычным управленческим решением,
          которое окупается: люди держатся за место, реже уходят и приводят
          знакомых. Мы жалеем только о том, что не начали заниматься этим на пару
          лет раньше — сомнения оказались куда меньше реальной пользы.
        </Quote>
        <Quote size="S" author="Имя Фамилия" role="Должность, НКО" avatars={1}>
          Quote · S — спокойная цитата без подложки, прямо на фоне страницы.
          Короткая в пять строк укладывается, поэтому «Далее» не появляется.
        </Quote>
      </CardContainer>
...
export function MediaSection() {
  return (
    <SectionContainer>
      ...
      <Text size="L">
        Video — ролик, например интервью героя. Кнопка play по центру, титры
        автора слева вверху; в прототипе — пастельный плейсхолдер без реального
        кадра.
      </Text>
      <CardContainer>
        <Video
