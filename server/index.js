...
/*
  ДОСТУП ОТКРЫТ — пароля нет, решение заказчика. Правки, комментарии и
  директивы может читать и менять любой, у кого есть ссылка. Круг узкий и
  доверенный, а лишний замок только мешает работе.

  Почему не «на всякий случай оставить»: экрана ввода пароля в приложении нет
  вовсе (checkAuth в src/editor-source/auth.ts никто не вызывает). С включённой
  проверкой запись отдавала 401, клиент сбрасывал токен и перезагружал
  страницу — редактировать на боевом стенде становилось нельзя в принципе.

  Страховка от потери данных — ежедневный бэкап data/. Удаление необратимо.
*/
const api = express.Router();

/*
  Замка нет, но маршрут нужен: этот адрес Railway дёргает как healthcheck
  (railway.json → healthcheckPath). Без него приходил 404 и деплой не вставал.
  Ответ читает и клиент — «пароль не требуется, доступ есть».
*/
api.get("/auth", (_req, res) => {
  res.json({ required: false, ok: true });
});
